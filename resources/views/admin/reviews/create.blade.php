...
@section('content')
    ...
    <form action="{{ route('admin.reviews.store') }}" method="POST" enctype="multipart/form-data"></form>
    @csrf
    {{-- form inserimento nuova recensione --}}
    <div class="mb-3 mt-3">
        <label for="review-title" class="form-label">Titolo</label>
        <input type="email" class="form-control" id="review-title" placeholder="titolo">
    </div>
    <div class="mb-3">
        ...
        <select class="form-select" id="game-select" name="game_id" aria-label="Seleziona il gioco">
            ...
            @foreach ($games as $game)
                <option value="{{ $game->id }}" data-image="{{ asset($game->image) }}">{{ $game->name }}</option>              
            @endforeach
        </select>
    </div>
    <div class="mb-3">
